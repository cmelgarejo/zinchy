...
vi.mock("@/lib/runtime-config", () => ({
  regenerateRuntimeConfig: vi.fn().mockResolvedValue(undefined),
}));
...
import { regenerateRuntimeConfig } from "@/lib/runtime-config";
import { db } from "@/db";
...
describe("PATCH /api/agents/[agentId]", () => {
  ...
  it("admin can update allowedTools for shared agent", async () => {
    ...
    vi.mocked(updateAgent).mockResolvedValueOnce({
      ...
      name: "Shared Agent",
      model: "anthropic/claude-sonnet-4-20250514",
      allowedTools: ["shell", "zinchy_ls"],
    } as never);

    const request = new NextRequest("http://localhost:7777/api/agents/agent-1", {
      method: "PATCH",
      body: JSON.stringify({ allowedTools: ["shell", "zinchy_ls"] }),
      headers: { "Content-Type": "application/json" },
    });
    const response = await PATCH(request, {
      params: Promise.resolve({ agentId: "agent-1" }),
  ...
  it("does not call regenerateRuntimeConfig directly (updateAgent handles it)", async () => {
    vi.mocked(auth).mockResolvedValueOnce({
      user: { id: "admin-1", role: "admin" },
      expires: "",
    } as ReturnType<typeof auth> extends Promise<infer T> ? T : never);
    ...
    expect(regenerateRuntimeConfig).not.toHaveBeenCalled();
  });
