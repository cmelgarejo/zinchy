...
describe("updateAgent", () => {
  ...
  it("should call regenerateRuntimeConfig after update", async () => {
    const { regenerateRuntimeConfig } = await import("@/lib/runtime-config");

    await updateAgent("1", {
      name: "Updated Smithers",
    });

    expect(regenerateRuntimeConfig).toHaveBeenCalled();
  });
  ...
  it("should accept allowedTools and pluginConfig in update data", async () => {
    ...
    expect(result.allowedTools).toEqual(["shell", "zinchy_ls"]);
  });
});
