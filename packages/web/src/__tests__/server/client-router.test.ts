...
function createMockRuntimeClient(connected = true) {
...
describe("ClientRouter", () => {
  ...
  let mockRuntimeClient: ReturnType<typeof createMockRuntimeClient>;
  ...
  beforeEach(() => {
    ...
    mockRuntimeClient = createMockRuntimeClient(true);
    router = new ClientRouter(mockRuntimeClient as any, "user-1", "user", sessionCache);
  ...
  it("should pass agentId and sessionKey to Runtime chat", async () => {
  ...
  it("should fetch history via runtimeClient.sessions.history", async () => {
    const clientWs = createMockClientWs();
  ...
  it("should return empty history when session not in cache", async () => {
    ...
    const freshRouter = new ClientRouter(mockRuntimeClient as any, "user-1", "user", freshCache);
    const clientWs = createMockClientWs();
  ...
  it("should wait for reconnect and succeed when Runtime reconnects in time", async () => {
    // Start disconnected — sessions.history throws like real client
    const disconnectedClient = createMockRuntimeClient(false);
  ...
  it("should return error after timeout when Runtime does not reconnect", async () => {
    vi.useFakeTimers();
    ...
    mockSessionsHistory.mockRejectedValue(new Error("Not connected to Runtime Gateway"));
  ...
  it("should use session key format agent:<agentId>:user-<userId> for per-user scoping", async () => {
    ...
    // The agentId segment must match Runtime's validation (agentId param == agentId in key).
  ...
  it("should find session when sessions.list returns user-scoped keys", async () => {
    // Sessions in Runtime use the format agent:<id>:user-<userId>.
    // The router must generate keys in the same format to find existing sessions.
    const freshCache = new SessionCache();
    const freshRouter = new ClientRouter(mockRuntimeClient as any, "user-1", "user", freshCache);

    // Runtime returns sessions with its native key format
    mockSessionsList.mockResolvedValue({
      sessions: [{ key: "agent:agent-1:user-user-1" }],
    });
    mockSessionsHistory.mockResolvedValue({
  ...
  it("should send error to client when stream yields an error chunk", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining("Runtime error chunk"),
