...
import type { AgentRuntimeClient, RuntimeChatChunk } from "@/types/runtime";
...
export class ClientRouter {
  ...
  async handleMessage(clientWs: WebSocket, message: BrowserMessage): Promise<void> {
    ...
    try {
      ...
      const stream = this.runtimeClient.chat(text, chatOptions);
      ...
      for await (const chunk of stream) {
        ...
        if (chunk.type === "error") {
          console.error("Runtime error chunk:", chunk.text);
          this.sendToClient(clientWs, {
            type: "error",
            message:
  ...
  private async handleHistory(
    ...
    try {
      ...
      // Check if session exists in runtime (cached)
      if (this.sessionCache.isStale()) {
        try {
          const result = await this.runtimeClient.sessions.list();
      ...
      if (!this.sessionCache.has(sessionKey)) {
        // Session doesn't exist in runtime yet — return greeting or empty
        const messages = agent.greetingMessage
          ? [{ role: "assistant", content: agent.greetingMessage }]
          : [];
        this.sendToClient(clientWs, { type: "history", messages });
      ...
      // Session exists — fetch history from runtime
      const result = await this.runtimeClient.sessions.history(sessionKey);
      ...
      const messages = rawMessages
        ...
        .map((msg) => {
          ...
          // Strip runtime timestamp prefix from user messages
          if (msg.role === "user") {
  ...
  private waitForConnection(): Promise<void> {
    if (this.runtimeClient.isConnected) return Promise.resolve();
    return new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        this.runtimeClient.removeListener("connected", onConnected);
        reject(
      ...
      this.runtimeClient.once("connected", onConnected);
    });
  }
