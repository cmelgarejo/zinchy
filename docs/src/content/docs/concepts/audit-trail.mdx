...
Zinchy includes a built-in audit trail that logs every significant action on the platform. Each entry is cryptographically signed with HMAC-SHA256 to detect tampering. The audit log is append-only — PostgreSQL triggers prevent any modification or deletion of existing entries.

The audit trail is designed for **compliance and security**. It answers the question: "Who did what, and when?"

## What gets logged

Zinchy logs 12 event types across five categories:
...
1. When an audit event occurs, Zinchy constructs a payload from the entry's fields (event type, actor, timestamp, metadata).
...
3. Zinchy recomputes HMAC signatures for all entries and reports any mismatches.
...
```bash
curl -b session_cookie https://your-zinchy-instance/api/audit/verify
```
...
```bash
curl -b session_cookie "https://your-zinchy-instance/api/audit/export?from=2026-01-01&to=2026-02-01" -o audit-log.csv
```
