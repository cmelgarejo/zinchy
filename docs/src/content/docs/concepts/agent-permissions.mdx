...
description: How Zinchy controls what each agent can do using an allow-list of tools and scoped directory access.
---

## Why agent permissions matter

AI agents are powerful — but in an enterprise setting, they must be constrained. An agent that can read any file on the server or execute arbitrary commands is a liability. Zinchy enforces **per-agent permissions** so each agent can only use the tools an admin explicitly enables.

## Allow-list approach

Zinchy uses an **allow-list** model: agents have **no tools by default**. An admin must explicitly enable each tool an agent is allowed to use. This is the opposite of a deny-list approach where everything is allowed unless blocked — with Zinchy, nothing is allowed unless granted.
...
Zinchy organizes tools into two categories:
...
OpenClaw uses a **deny-list** model internally: by default every tool group is available, and you block what you don't want. Zinchy's allow-list is the opposite — nothing is available until enabled.

To bridge these two models, Zinchy converts the allow-list into a deny-list at config generation time using `computeDeniedGroups()`. This function takes the list of tool IDs an admin has enabled and returns all tool groups that should be blocked. The result is written into each agent's `tools.deny` array in the OpenClaw config.

For example, if an admin enables only `zinchy_ls` and `zinchy_read` (safe tools), `computeDeniedGroups` returns all powerful tool groups (`computer`, `files`, `web_search`, `web_fetch`) so that OpenClaw blocks those capabilities for this agent.
...
| ------------------ | --------------------------------------- | -------------------------------------------------- |
| **Knowledge Base** | `zinchy_ls`, `zinchy_read` (safe tools) | Answer questions from selected documents           |
...
Zinchy does not rely on any single layer for security. Four layers work together:
...
3. **Plugin path validation** — the `zinchy-files` plugin checks every requested path against the agent's allowed directories
...
Not every user can see every agent. Zinchy enforces access rules:
...
All agent-accessible files live under `/data/` inside the Zinchy container. This is mounted as a Docker volume, and you can bind-mount host directories into subdirectories of `/data/`.
