...
- Zinchy is running (see [Quick Start](/getting-started/))
...
Before creating a Knowledge Base agent, make sure the documents you want the agent to access are mounted under `/data/` in the Zinchy container. See the [Mount Data Directories](/guides/mount-data-directories/) guide for detailed instructions.

## 2. Navigate to "New Agent"

In the Zinchy sidebar, click the **+** button next to "Agents" to open the agent creation form.
...
Under the hood, Zinchy uses an allow-list approach. The agent starts with no capabilities and only gets the tools an admin explicitly enables. For a full explanation, see [Agent Permissions](/concepts/agent-permissions/).
