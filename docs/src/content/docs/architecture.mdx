---
title: Architecture
description: How Zinchy wraps OpenClaw, the request flow, and the tech stack.
---

## Overview

Zinchy is **not a fork** of OpenClaw. It's a governance layer on top of it. OpenClaw handles agent execution, tool use, and model communication. Zinchy adds authentication, provider management, agent permissions, cryptographic audit trails, and (soon) team management.
...
graph TD
    ...
    subgraph Zinchy["Zinchy (Node.js)"]
...
1. **Browser** → connects via WebSocket to `/api/ws` on Zinchy, sending a JSON message with `type`, `content`, and `agentId`
2. **Zinchy** → authenticates the user, verifies agent access, resolves the **session key** from the in-memory session cache, generates a message ID, and forwards the message to OpenClaw Gateway with the `agentId` and `sessionKey`
...
4. **Zinchy** → attaches the message ID and forwards each chunk to the browser
5. **Browser** → renders the streaming response in real time

Each browser connection gets its own `ClientRouter` instance that manages agent access checks and session resolution. Zinchy acts as a bridge — it never interprets or modifies the AI response content.
...
The `agentId` segment must always be the Zinchy agent UUID. The `scope` segment is free-form but should follow the patterns above for consistency.
...
Zinchy uses [Auth.js v5](https://authjs.dev/) with a credentials provider:
...
2. Zinchy generates a random token, stores its **SHA-256 hash** in the database, and returns the plaintext token as a one-time invite link
...
Zinchy uses an **allow-list** model for agent permissions: agents have **no tools by default**. Admins explicitly enable tools for each agent via the Permissions tab in Agent Settings.

Tools are organized into two categories:

- **Safe tools** — sandboxed directory access (`zinchy_ls`, `zinchy_read`). Only work within directories the admin has approved.
- **Powerful tools** — direct server access (shell commands, unrestricted file access, web access). Only for trusted use cases.

When an agent has safe tools enabled, the `zinchy-files` plugin validates every file access request against the agent's allowed directories, with symlink resolution to prevent escapes.
...
Zinchy restricts which agents a user can see:
...
- A 256-bit encryption key is either provided via the `ENCRYPTION_KEY` environment variable or auto-generated and persisted in the `zinchy-secrets` Docker volume
- Each encrypted value stores the IV, auth tag, and ciphertext together
- Decryption happens on-demand when Zinchy writes the OpenClaw configuration file

## Audit Trail

Zinchy includes a cryptographic audit trail for compliance and security. Every significant action is logged to the `auditLog` table with an HMAC-SHA256 signature.
...
Zinchy logs 12 event types covering authentication, agent management, user management, configuration changes, and tool execution. Chat message content is **not** logged — only tool calls and permission-relevant actions.
...
OpenClaw runs as a separate Docker container. Zinchy communicates with it via `openclaw-node` (the official Node.js client) over WebSocket on port 18789. The browser never connects to OpenClaw directly.

### Config generation

Zinchy owns the OpenClaw configuration file (`openclaw.json`). Config generation happens in two scenarios:
...
Zinchy authenticates to OpenClaw Gateway using a bearer token. The token is auto-generated on first setup via `crypto.randomBytes(24)` and stored in the `gateway.auth` block of `openclaw.json`. It never appears in source control.
